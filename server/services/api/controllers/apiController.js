            ...
            fetch(matchListUrl)
                .then(function(ml_json) {
                    ...
                    for (var j = 0; j < matchCount; j++) {
                        ...
                        var api_url = serverName + ':' + port + '/api/getmatchtimeline/' + matchID + '/' + account_id;
                        fetch(api_url)
                            .then(function(death_json) {
                                //console.log(death_json);
                                //res.send(death_json.json());
                                console.log('death_json: ' + death_json);
                            });
                        }
                    });
            });
...
exports.getmatchtimeline = function (req, res) {
    var deathObjList = [];
    ...
    fetch(participantUrl)
        .then(function(partipant_json) {
            ...
            console.log('participant data: ' + participant_data.participantIdentities);
            if (participant_data.hasOwnProperty('participantIdentities')) {
                if (participant_data.participantIdentities[0].hasOwnProperty('player')) {
                var participantID = -1;
                if (participant_data.participantIdentities[0].player.hasOwnProperty('accountId')) {
                    ...
                    fetch(timeLineUrl)
                        .then(function(timeline_json) {
                            ...
                            if (timeline_data.hasOwnProperty('frames') && timeline_data.frames.length > 0) {
